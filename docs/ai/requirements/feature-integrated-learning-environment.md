...
* **User's Action:** Learn via hierarchical questioning and active recall, while the structural Learning Book (Markdown) is updated in real time as a byproduct of the conversation—no manual extract/format/paste. The user is encouraged to respect the Subject Roadmap (A) so progression is deliberate, not ad hoc. **As Learner, the User relies on the Agent to act as a world-class professional in the subject being learned; the Agent asks clarifying questions (current understanding, desired outcome) to guide best; the Agent generates possibilities (from its knowledge and/or Phase B source materials) and the Learner contemplates and chooses which possibilities to append to the page.**
...
* **Principles (Why):**
  ...
  * **Agent as teacher:** The Agent acts as a world-class professional in the subject being learned; asks clarifying questions (Learner's current understanding, desired outcome) to tailor guidance; generates possibilities (from its knowledge and/or Phase B. Capture Facts & Data) and presents them so the Learner can contemplate and choose which possibilities to append to the page—no append without Learner choice or approval.

* **Environment (Where):** Digital, local-first. Primary artifact: a Markdown repository (Learning Book) on the user's machine. Structure: LTC COE → Areas of Expertise (Effectiveness, Data Science, WellBeing, Business Excellence, Investment, Technology, Leadership, …) → Chapters → Topics. **The company-side COE hierarchy (COE → Chapter → Topic → Topic Members' Learning Area → Personal Learning Area) is defined in `config/coe-map.yaml`** (single source of truth; T-411). Per subject: A. Subject Roadmap & Level Specifications (SFIA L1–L7); B. Capture Facts & Data; C. Organise Information (components: 0. Overview, 1. UBS, 2. UDS, 3. EPS, 4. UES, 5. EOP, …); D. Distill Understanding; E. Express Expertise. Primary interfaces: Cursor Chat and/or AntiGravity; optional later: NotebookLM-like capabilities (Audio Overview, Infographics, Quiz) with the constraint that the system must allow exploration outside given sources.
