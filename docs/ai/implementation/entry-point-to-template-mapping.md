...
_Rule: when user selects entry point X, Agent loads template Y and scopes conversation/document context to that entry. Aligned with `learning-book-tree-map.md`._
...
_Deterministic behavior when user selects an entry point._
...
3. **Load template** — Read the template file at the resolved path. Use its structure (questions × components, target table format) for the conversation and for any proposed doc updates. If the file does not exist, use the structure from `templates/page-0-overview-and-summary.md`. **Target file and folder names** must follow the Learning Book naming convention: `docs/ai/implementation/ile-learning-book-naming-convention.md` (prefix every phase folder, topic folder, and page file with `[COE AREA]_[MEMBER]_SUBJECT NAME - `).
...
_For Phase C. Organise Information topic content: resolve template by **page** so every section (36 × 7 = 252) is covered by exactly one of 7 page templates. See `docs/ai/implementation/ile-template-mece-checklist.md`._
...
_All 7 page templates exist. Use this table when (subject, phase, chapter, topic, page) is known; target file/section still resolved per `ile-conversation-to-doc-mapping.md`. Load the template **before** generating content for this page type._

## Phase B (B. Capture Facts & Data)

_Phase B is **one file per subject** (not per chapter/topic). The file contains a single table: USER GUIDE + rows for chapters 0–5 with subtopics and learning-tool columns. No (chapter, topic, page) entry-point resolution for B—the whole subject is one entry._

| Phase | Entry                                   | Template path                                   |
| ----- | --------------------------------------- | ----------------------------------------------- |
| B     | B. Capture Facts & Data (subject-level) | `templates/B-captured-facts-and-information.md` |

_When user selects Phase B, load `templates/B-captured-facts-and-information.md` and scope context to the single B file for the current subject per `ile-conversation-to-doc-mapping.md`._

## Mapping table (legacy / A. Subject Roadmap)

_6 Chapters × 6 Topics. Each Topic has 7 pages (0–5 + Page 7 Topic Distilled Understanding). Organise level = 16 columns (14 questions + 2 notes). Distilled level = 17 columns (14 questions + 3 notes)._
...
_For A. Subject Roadmap use the first row. For Phase C topic content use **page-type resolution** above (7 page templates)._
