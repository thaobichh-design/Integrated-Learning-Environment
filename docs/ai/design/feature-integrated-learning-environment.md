...
_Approved in State A Sub-Step 2. Source: `docs/ai/requirements/feature-integrated-learning-environment.md`._
...
### 1.7 Level practice, proof, and rubrics (extension — I3/I4)

**Goal:** Support level-appropriate practice and proof capture without hard-coding level→topic maps. A remains the single source of truth for Level → Phase C scope, rubrics, and proof landing zone. _Source: Requirements Phase 1 & 2 extension; A.C. Verb-AC11, Verb-AC12, SustainAdv-AC4, EffAdv-AC5, ScalAdv-AC6, Noun-AC14, Noun-AC15, SustainAdj-AC6, EffAdj-AC6._

- **Proof landing zone in A:** Per subject, A supports a defined place where evidence for each level is stored or linked (e.g. links to artefacts, checklist items). Same structure for Agent-assisted self-check and future external assessor (SustainAdv-AC4, Noun-AC14).
- **Rubrics per level in A:** A defines pass/fail criteria per credential per level; Agent uses these for rubric walk-through and proof recording; future assessor can use the same criteria (SustainAdv-AC4, Noun-AC14, SustainAdj-AC6).
- **Level → Phase C scope in A:** A defines which Phase C content (e.g. "Links to Learning Book (per Level)") belongs to which level. No hard-coded level→topic map in ILE; Agent reads from A (Noun-AC15, ScalAdv-AC6).
- **Agent behaviour:** (1) **Self-check before practice (T-305):** When Phase C scope for a level is complete or when the User asks, Agent can offer a comprehensive self-check from A and Organise content; optional, Agent suggests and does not block (Verb-AC11, EffAdj-AC6). (2) **Rubric walk-through and proof recording (T-306):** When the User returns from practice, Agent can walk through the level rubric and help record evidence in the proof landing zone and update the Level Completion Checklist; proof capture in-conversation (Verb-AC12, EffAdv-AC5). (3) **Order and gates (T-415):** Agent reads from A Level → scope and any stated gates (e.g. L_n credentials before L_n+1) and suggests sequence; no hard-coded map in ILE.

---

# 2. TECHNICAL ARCHITECTURE (The Noun)

_First pass from Requirements and Planning; refined in State B as we implement each iteration._
...
_How the feature attributes enable the user to reach the Effectiveness Outcomes._

_Map each attribute below to the corresponding Requirements A.C. IDs and to the Planning iteration where that A.C. is validated. Design defines how; Requirements and Planning define what and when._

- **Sustainability (Risk/Safety):** Structure-faithful — SustainAdj-AC1..AC5. _Validated in:_ Iteration 2 (Working Prototype) for AC1–AC3; Iteration 4 (Leadership) for AC4–AC5.
...
- **Efficiency (Speed/Utility):** Zero-friction capture — EffAdj-AC1..AC3. _Validated in:_ Iteration 3 (MVE).
...
- **Scalability (Growth):** Template-driven — ScalAdj-AC1..AC4. _Validated in:_ Iteration 4 (Leadership).
...
- **Usage analytics (I4, options open):** ScalAdv-AC5, Noun-AC10. _Validated in:_ Iteration 4 (Leadership).
...
_First-pass estimates from Requirements and Planning. Update when architecture or scope changes._
...
_Ongoing tracking:_ See Planning §3 (Resource & Budget Tracker) for current usage vs limits.
...
_Last updated: §1.7 Level practice, proof, rubrics (extension). §1.6 Operational hardening + §3 Sustainability (SustainAdj-AC4, AC5). Aligned with template 1.1.0. Planning: `docs/ai/planning/feature-integrated-learning-environment.md`. Full design (§2–4) refined in State B._
